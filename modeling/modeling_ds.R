...
confusionMatrix(table(modelrf_imp$predicted,df_train$TARGET)) #Accurecy Score: 0.6131

# ROC for important variables
pred_train <- predict(modelrf_imp,df_train,type = 'prob')
pred_train <- prediction(pred_train[,2],df_train$TARGET)
confusionMatrix(table(pred_test,df_test$TARGET)) #Accurecy score: 0.5203

roc <- performance(pred_train,"tpr","fpr")
plot(roc,colorize=T,main="ROC curve",xlab="1-Specificity",ylab="Sensitivity")
abline(a=0,b=1)

# Cross validation for imporant variables
control <- trainControl(method='repeatedcv', number=3, repeats=1, search='grid')

tunegrid <- expand.grid(.mtry = c(4,6))

rf_gridsearch <- train(TARGET ~ ., data = df_train, method = 'rf', ntree = 100, metric = 'Accuracy', tuneGrid = tunegrid)

# Accuracies with various mtry
print(rf_gridsearch)

pred_test = predict(rf_gridsearch$finalModel,newdata = df_test)
confusionMatrix(table(pred_test,df_test$TARGET)) #Accuracy Score: 0.5188

# to plot the graph for accuracies
plot(rf_gridsearch)
